  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 p-4 space-y-3">
      <label className="block">
        <span className="text-xs font-semibold text-stone-500 uppercase tracking-wide">Subcontractor / Company Name</span>
        <input
          type="text"
          value={name}
      ...
      <label className="block">
        <span className="text-xs font-semibold text-stone-500 uppercase tracking-wide">Contact Emails</span>
        <textarea
          value={emails}
      ...
      <div className="flex gap-2">
        ...
        <button onClick={onCancel} className="px-4 py-2.5 rounded-lg border border-stone-300 text-stone-600 font-semibold text-sm">
          Cancel
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-stone-100 font-sans">
      <div className="max-w-md mx-auto pb-10">
        <header className="sticky top-0 bg-white border-b border-stone-200 px-4 py-3 flex items-center gap-3 z-10">
          <button onClick={() => setView("settings")} className="p-1.5 -ml-1.5 rounded-full hover:bg-stone-100">
            <ChevronLeft size={22} />
          </button>
          <h1 className="font-semibold text-stone-800">Subcontractor Contacts</h1>
        </header>
        ...
        <div className="p-4 space-y-4">
          ...
          {subcontractors.length === 0 && !adding && (
            <div className="text-center py-12 px-6">
              <div className="w-16 h-16 mx-auto rounded-full bg-stone-200 flex items-center justify-center mb-4">
                <Building2 size={28} className="text-stone-400" />
              </div>
              <h3 className="font-semibold text-stone-700 mb-1">No contacts yet</h3>
              <p className="text-sm text-stone-400">Add a subcontractor to start routing reports to them automatically.</p>
            </div>
          )}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="font-semibold text-stone-800 flex items-center gap-2">
                        <Building2 size={16} className="text-teal-600" /> {s.name}
                      </div>
                      <div className="mt-2 space-y-1">
                        {(s.contact_emails || []).length === 0 ? (
                          <p className="text-xs text-stone-400">No emails added</p>
                        ) : (
                          s.contact_emails.map((e) => (
                            <div key={e} className="flex items-center gap-1.5 text-xs text-stone-500">
                              <Mail size={12} /> {e}
                            </div>
                          ))
                        )}
                    ...
                    <div className="flex gap-1 shrink-0">
                      <button onClick={() => setEditingId(s.id)} className="text-stone-400 hover:text-stone-700 p-1.5">
                        <Building2 size={16} />
                      </button>
                      <button onClick={() => deleteSubcontractor(s.id)} className="text-red-400 hover:text-red-600 p-1.5">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
          ...
          {!adding && (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-stone-300 text-stone-500 font-semibold py-3 rounded-xl hover:border-teal-400 hover:text-teal-600 transition-colors"
            >
